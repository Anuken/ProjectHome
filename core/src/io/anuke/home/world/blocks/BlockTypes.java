			...
			if(useEdge){
				...
				for(GridPoint2 point : Geometry.getD8Points()){
					...
					if(Math.abs(dx) > 0 && Math.abs(dy) > 0){
						...
						if(World.get(tile.x + dx, tile.y).floor.id != id ||
								World.get(tile.x, tile.y + dy).floor.id != id){
							continue;
						}
					}
				...
				for(GridPoint2 point : Geometry.getD4Points()){
					Tile other = World.get(tile.x + point.x, tile.y + point.y);
				...
				for(GridPoint2 point : Geometry.getD8EdgePoints()){
					...
					if(World.get(tile.x + point.x, tile.y).floor.id == id) overlaps ++;
					if(World.get(tile.x, tile.y + point.y).floor.id == id) overlaps ++;
	...
	public static class Bookshelf extends Wall{
		...
		public void draw(FacetList list, Tile tile){
			...
			new BaseFacet(tile.worldy() - Vars.tilesize / 2, Sorter.object, p -> {
				...
				if(!World.isWall(tile.x, tile.y - 1)){
